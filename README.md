...
A **Program** is one help channel + one private "BTS" channel (daily summaries, and its membership is what makes someone a helper) + its own welcome message, FAQ link, admin-panel link, and quick-close **quick replies**. Everything below happens per-Program.

- **Super admin** (`SUPER_ADMIN_USER_ID` in `.env`, comma-separated, the one thing that still lives outside the database since nothing in an empty database can decide who's allowed to create the first row in it): adds Programs from the Home tab's **admin** tab, and can edit any Program (its channels, its admin, and everything a Program admin can). Can see every Program.
- **Program admin** (one user, assigned by a super admin): can edit everything about their own Program from an **edit program** button next to the tab switcher -- its channels, who the admin is, welcome message, FAQ link, admin-panel link, and the quick replies list (add/edit/delete, no code changes needed).
- **Helper** (member of a Program's BTS channel): claims/resolves/reopens/wipes tickets in that Program same as always. The BTS channel, not a ping usergroup, is the source of truth here on purpose, since anyone can add themselves to a usergroup but a super admin actually has to invite people into the BTS channel.
...
- Either the **ticket opener** or anyone in that Program's **BTS channel** can resolve or reopen a ticket
...
- A tiny, staff-only overflow menu on the greeting ("Support Scouts only") opens a modal for that Program's helpers with the opener's ticket stats, a one-click link into the admin panel (if the Program has one set, filled in with `{userId}`/`{email}`), canned quick-close replies, and a **wipe thread** button
- Quick replies can ping anyone, bots included, via a "ping a user or bot" picker in the add/edit form that inserts the mention for you, no need to know a bot's raw Slack ID
- A super admin can set **default quick replies** that get copied onto every new Program when it's created (a snapshot, not a live link, so a Program can freely edit or remove its own copy afterward without touching the defaults)
...
src/
  ...
  db/
    ...
    programs.ts              Program CRUD, isSuperAdmin
    quickReplies.ts          per-Program quick-close reasons (CRUD)
    defaultQuickReplies.ts   super-admin-managed defaults, copied onto every new Program
  ...
  utils/
    ...
    slugifyKey.ts            label -> quick-reply key
  slack/
    app.ts                   Bolt app instance (Socket Mode)
    helpers.ts               BTS-channel membership lookup + cache, parameterized per Program
    userName.ts              friendly display name lookup, never a mention
    userEmail.ts             email lookup for admin-panel link templating, needs users:read.email
  features/
    tickets/
    ...
    home/
      ...
      programAdminModals.ts      one add/edit Program modal (channels, admin, copy, quick replies inline), default quick replies modal
...
7. In Slack, open Hestia's **Home tab** -> **admin** -> **add program**: pick the help channel, the BTS channel (whoever's in it are that Program's helpers), and who the Program's admin should be. Invite the bot to both channels first (`/invite @hestia`), it can't see or post in a channel it isn't a member of, private channels included.
8. That Program's admin (or a super admin) can now edit everything about it, including its welcome message, FAQ link, admin-panel link, and quick replies, from the **edit program** button next to the tab switcher on the overview tab, no redeploy needed.
...
If you were already running Hestia before Programs existed, keep your old `SUPPORT_CHANNEL_ID` / `SUMMARY_CHANNEL_ID` / `FAQ_CANVAS_URL` / `STARDANCE_ADMIN_URL` values in `.env` (see the bottom of `.env.example`). On first boot, if no Program exists yet, `src/db/legacyMigration.ts` turns them into your first Program automatically (admin defaults to your first `SUPER_ADMIN_USER_ID`, and its two `fraud`/`hackatime` quick replies are seeded from what used to be hardcoded, plus anything already in the default quick replies list). It also backfills `program_id` on every ticket that predates that column, on every boot, matched by channel, so tickets you already had open keep working with resolve/reopen/claim instead of silently losing their Program. Once that Program exists, those env vars are dead weight, feel free to delete them, everything about it now lives in the database and the admin tab.
...
- **Resolving:** clicking the button checks that the clicker is either the opener or a member of that Program's BTS channel (looked up live via `conversations.members`, cached 5 minutes per channel). If allowed, the ticket is marked resolved, the original threaded reply is rewritten with the button removed, a brand new message announces who resolved it with a Reopen button (`resolution_ts` tracks that message), and the reaction on the original message flips from :thinking_face: to :white_check_mark:.
...
- **Staff-only user info:** the small overflow menu (⋮, labeled "Support Scouts only") on the greeting message checks membership in that Program's BTS channel before doing anything; anyone else (opener included) gets an ephemeral "staff only" reply posted right in the ticket's thread. Helpers get a modal with the opener's ticket stats within that Program and, if the Program has `admin_url_template` set, a link into it with `{userId}` (the opener's Slack ID) and `{email}` (needs `users:read.email`) filled in.
...
- **Quick close:** the same modal lists buttons from that Program's `quick_replies` rows, each one resolves the ticket (no reopen button this time) and posts its exact message as the resolution announcement instead of the usual "resolved by X" line, so it never names which Scout clicked it (they're still credited internally for the leaderboard) and there's no celebratory wording either. A Program's admin manages this list (add/edit/delete, plus a picker to ping a user or bot without typing their raw Slack ID) inline in **edit program**, no code changes needed. A super admin's **default quick replies** (from the admin tab) get copied onto every new Program automatically.
...
- **Admin** (super admins only): every Program (name, channels, admin) with an "edit" button per row, an "add program" button, and a **default quick replies** button.

If you're that Program's admin or a super admin, an **edit program** button sits right next to the tab switcher on the overview/mine tabs, opening the one modal for everything about that Program: its channels, its admin, its copy, and its quick replies inline underneath. A Program picker (shown whenever more than one Program exists, for anyone, not just its helpers) sits above the overview/mine content, since the overview stats are public info.
